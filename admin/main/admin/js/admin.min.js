  ...
  $.getScript('/js/api.js');
    ...
    $( ".gen-report" ).click( () => {
	  ...
	  $( ".hm-btn-modal-e1" ).click( () => {
         // hilmacs default comment
				 var x = ({
					 title:$('.hm-inpt-modal-e1').val(),
					 img:$('.hm-inpt-modal-e2').val(),
					 units:$('.hm-inpt-modal-e3').val(),
					 desc:$('.item-description').val(),
					 price:$('.item-price').val(),
					 categories:$('.item-categories').val().split(",")
			   });
			   console.log(x)
			   //return
				 if (x.title && x.img) {
           $.post(Path.gate('products',null,'/new'), x ).done( () => {
           	Service.Products();
           });
				 } else {
           alert(Lang.locals('noChg'));
				 }
     });
     ...
     $( ".hm-btn-modal-s1" ).click( () => {
         // hilmacs default comment
				 var x = ({
					 ...
					 img:$('.hm-inpt-modal-s2').val()
			   });
				 if (x.name) {
           $.post(Path.gate('categories',null,'/new'), x ).done( ()=> {
           	 Service.Categories();
           });
				 } else {
	  ...
	  $( ".btn-xs" ).click( () => {
           ...
           $.post(Path.gate('users',null,'/delete'), { } ).done( (data)=> {
			   console.log(data)
